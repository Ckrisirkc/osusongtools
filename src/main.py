...
def setSongsFolder(desc=False):
    ...
    if desc:
        ...
        if not(SONGS_FOLDER is None):
            rtn += "; Currently '" + os.path.abspath(SONGS_FOLDER) + "'"
        return rtn
    
    if (os.path.isdir("C:\\Program Files (x86)\\osu!\\songs") and SONGS_FOLDER is None):
        sf = "C:\\Program Files (x86)\\osu!\\songs"
    else:
        root = Tk()
        root.withdraw()
        sf = tkFileDialog.askdirectory(parent=root, initialdir=os.getcwd(), title="Please select your osu! songs folder")
    SONGS_FOLDER = sf
    return True
...
def changeExportFolder(desc=False):
    ...
    root = Tk()
    root.withdraw()
    ef = tkFileDialog.askdirectory(parent=root, initialdir=os.getcwd(), title="Please select the folder you wish to export to")
    EXPORT_FOLDER = ef
    return True
